...
class DocumentService:
    ...
    def get_document_raw(
        ...
        """读取文档当前生效版本的原始 Markdown，供前端预览与编辑。"""

        rows = self.milvus.query(
            build_document_filter(user.tenant_id, document_id, active_only=True)
        )
        if not rows:
            raise NotFoundError("没有找到这个文档。")

        source_path = str(rows[0]["source_path"])
        full_path = self.storage_root.joinpath(*source_path.split("/"))
        if not full_path.is_file():
            raise NotFoundError("原始 Markdown 文件不存在。")

        summary = self.summarize(rows)[0].to_api()
        return {
            **summary,
