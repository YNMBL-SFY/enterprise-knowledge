...
class DocumentServiceTest(unittest.TestCase):
    ...
    def test_get_document_raw_reads_active_version_file(self):
        service = self.create_service([active_row()])
        service.write_source(
            "bluewhale/doc-1/v1.md", "# 退款规则\n\n退款金额超过 2000 元时，需要人工审核。"
        )

        raw = service.get_document_raw(USER, "doc-1")

        self.assertEqual(raw["documentId"], "doc-1")
        self.assertEqual(raw["version"], 1)
        self.assertEqual(raw["sourcePath"], "bluewhale/doc-1/v1.md")
        self.assertIn("退款金额超过 2000 元", raw["markdown"])

    def test_get_document_raw_requires_active_version(self):
        # 只有失效版本（已删除文档）时，不应返回原始 Markdown。
        service = self.create_service([active_row(is_active=False, version=1)])
        with self.assertRaises(NotFoundError):
            service.get_document_raw(USER, "doc-1")
